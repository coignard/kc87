...
pub const FB_WIDTH: usize = SCREEN_WIDTH + 2 * BORDER_X;
...
impl VideoRenderer {
    pub fn new(font_rom: Vec<u8>) -> Self {
        Self {
            font_rom,
        ...
        if bus.graph_robotron_active() {
            ...
            for ay in 0..SCREEN_HEIGHT {
                ...
                for ax in 0..SCREEN_WIDTH {
                    ...
                    let idx = (fb_y * FB_WIDTH + BORDER_X + ax) * 4;
        ...
        for ay in 0..SCREEN_HEIGHT {
            ...
            if glyph_line < 8 && row < rows {
                for col in 0..COLUMNS {
                    ...
                    let char_code = video_ram[offset] as usize;
                    ...
                    let (bg, fg) = if !has_color {
                        ...
                        let mut color_attr = color_ram[offset];
                    ...
                    for px in 0..8 {
                        let color = if (pixels & (0x80 >> px)) != 0 { fg } else { bg };
                        let idx = base_idx + px * 4;
                        self.frame_buffer[idx..idx + 4].copy_from_slice(&color);
                    }
                }
            } else if !has_color {
